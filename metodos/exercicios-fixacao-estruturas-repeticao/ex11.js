...
function ehPrimo(numero) {
  ...
  } else if (typeof numero !== 'number' || numero < 2 || !Number.isInteger(numero)) {
    return console.log(`O valor "${numero}" não é um número inteiro válido para verificação de primalidade.`);
  }
  for (let i = 2; i <= Math.sqrt(numero); i++) { // Verifica divisores até a raiz quadrada do número
    if (numero % i === 0) {
