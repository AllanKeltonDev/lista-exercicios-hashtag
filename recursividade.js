...
function calcularVendas(dia = 1) { // Função recursiva para calcular as vendas diárias
  if (dia > diasDoMes) {
    const vendasMes = vendasDiarias.reduce((total, vendas) => total + vendas * precoHamburguer, 0); // Calcula o total das vendas no mês
  ...
  console.log(`Vendas do dia ${dia}: R$${(vendasHoje * precoHamburguer).toFixed(2)}`);
}
